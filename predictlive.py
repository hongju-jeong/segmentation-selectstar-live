...
### RUN OPTIONS ###
MODEL_PATH = "./run/surface/deeplab/experiment_8/checkpoint.pth.tar"
ORIGINAL_HEIGHT = 480
ORIGINAL_WIDTH = 640
...
class FrameGeneratorJpg:
    ...
    def __iter__(self):
        for file in self.files:
            img = cv2.imread(file, cv2.IMREAD_COLOR)   #파일에 있는 이미지
...
def main():
    ...
    for index, img in enumerate(tqdm(generator)):   #tqdm 파이썬 프로세스 바
